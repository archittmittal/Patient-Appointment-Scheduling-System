...
app.get('/api/fix-db', async (req, res) => {
    ...
    try {
        ...
        // 1. Describe doctors table
        try {
            const [cols] = await db.query('DESCRIBE doctors');
            output += 'Doctors Columns: ' + cols.map(c => c.Field).join(', ') + '; ';
        } catch (e) {
            output += 'Describe doctors failed: ' + e.message + '; ';
        }

        // 2. Fix users table (Google OAuth compatibility)
        try { await db.query(`ALTER TABLE users MODIFY password_hash VARCHAR(255) NULL`); output += 'users password_hash altered; '; } catch(e) { output += 'users password_hash: ' + e.message + '; '; }
        try { await db.query(`ALTER TABLE users ADD COLUMN auth_provider VARCHAR(50) DEFAULT 'LOCAL'`); output += 'users auth_provider added; '; } catch(e) { output += 'users auth_provider: ' + e.message + '; '; }
        try { await db.query(`ALTER TABLE users ADD COLUMN google_id VARCHAR(255)`); output += 'users google_id added; '; } catch(e) { output += 'users google_id: ' + e.message + '; '; }
        try { await db.query(`CREATE UNIQUE INDEX idx_users_google_id ON users (google_id)`); output += 'users google_id index added; '; } catch(e) { output += 'users google_id index: ' + e.message + '; '; }

        // 3. Optional: Run all schema migrations
        if (req.query.migrate === 'true') {
            const fs = require('fs');
            const path = require('path');
            const migrationFiles = [
                'database/migration_week4_billing.sql',
                'database/migration_week4_messaging.sql',
                'database/migration_sprint2_schema_hardening.sql',
                'database/migrations/migration_sprint3_consultation_fee.sql',
                'database/migrations/migration_sprint4_otp_hardening.sql',
                'database/migrations/migration_sprint10_uppercase_status.sql',
                'database/migration_issue222_google_auth.sql'
            ];
                ...
                if (fs.existsSync(filePath)) {
                    ...
                    for (const statement of statements) {
                        try {
                            ...
                            if (err.code === 'ER_DUP_FIELDNAME' || err.code === 'ER_TABLE_EXISTS_ERROR' || err.code === 'ER_DUP_KEYNAME' || err.code === 'ER_CANT_DROP_FIELD_OR_KEY') {
        ...
        res.send('Database debug and fix result: ' + output);
    } catch (e) {
