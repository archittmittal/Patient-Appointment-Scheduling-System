...
class BatchingService {
    constructor() {
        ...
        this.getBatchableTypes = this.getBatchableTypes.bind(this);
        this.getBatchSlots = this.getBatchSlots.bind(this);
        this.createBatchSlot = this.createBatchSlot.bind(this);
        this.bookBatchAppointment = this.bookBatchAppointment.bind(this);
        this.getPatientBatchAppointments = this.getPatientBatchAppointments.bind(this);
        this.getDoctorBatchSchedule = this.getDoctorBatchSchedule.bind(this);
        this.getBatchSlotDetails = this.getBatchSlotDetails.bind(this);
        this.suggestBatchSlots = this.suggestBatchSlots.bind(this);
        this.cancelBatchSlot = this.cancelBatchSlot.bind(this);
        this.getBatchAnalytics = this.getBatchAnalytics.bind(this);
    }

    /**
     * Get appointment types that can be batched
     */
    async getBatchableTypes() {
     ...
     */
    async getBatchSlots(doctorId, date) {
        const sql = `
            SELECT bs.*, 
     ...
     */
    async createBatchSlot(doctorId, data) {
        const { batchType, date, startTime, maxCapacity, notes } = data;
        
        // Get batch type info
        const batchTypes = await this.getBatchableTypes();
     ...
     */
    async bookBatchAppointment(patientId, batchSlotId, reason) {
     ...
     */
    async getPatientBatchAppointments(patientId) {
        const sql = `
     ...
     */
    async getDoctorBatchSchedule(doctorId, startDate, endDate) {
        const sql = `
            SELECT bs.*,
     ...
     */
    async getBatchSlotDetails(slotId) {
        const [slots] = await db.execute(`
     ...
     */
    async suggestBatchSlots(patientId, appointmentType, preferredDate) {
     ...
     */
    async cancelBatchSlot(slotId, doctorId, reason) {
        // Verify ownership
        const [slots] = await db.execute(`
     ...
     */
    async getBatchAnalytics(doctorId, startDate, endDate) {
        try {
