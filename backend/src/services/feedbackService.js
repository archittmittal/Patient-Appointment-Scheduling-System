...
class FeedbackService {
    ...
    constructor() {
        ...
        this.getDoctorFeedbackAnalytics = this.getDoctorFeedbackAnalytics.bind(this);
        this.getSystemFeedbackAnalytics = this.getSystemFeedbackAnalytics.bind(this);
        this.getPendingFeedbackRequests = this.getPendingFeedbackRequests.bind(this);
        this.getFeedbackCategories = this.getFeedbackCategories.bind(this);
        this.getPatientFeedbackHistory = this.getPatientFeedbackHistory.bind(this);
    }

    /**
     * Submit feedback for an appointment
     */
    async submitFeedback(appointmentId, patientId, feedbackData) {
        ...
        for (const [category, rating] of Object.entries(ratings)) {
            if (FeedbackService.FEEDBACK_CATEGORIES[category]) {
                totalScore += rating * FeedbackService.FEEDBACK_CATEGORIES[category].weight;
                totalWeight += FeedbackService.FEEDBACK_CATEGORIES[category].weight;
            }
        }
    ...
    analyzeSentiment(text) {
        ...
        FeedbackService.SENTIMENT_KEYWORDS.positive.forEach(word => {
            if (lowerText.includes(word)) positiveCount++;
        });
        FeedbackService.SENTIMENT_KEYWORDS.negative.forEach(word => {
     ...
     */
    async getDoctorFeedbackAnalytics(doctorId, startDate, endDate) {
        try {
            // Overall stats
            ...
            Object.keys(FeedbackService.FEEDBACK_CATEGORIES).forEach(cat => {
            ...
            const categoryBreakdown = Object.entries(categoryScores).map(([cat, data]) => ({
                category: cat,
                label: FeedbackService.FEEDBACK_CATEGORIES[cat]?.label || cat,
     ...
     */
    async getSystemFeedbackAnalytics(startDate, endDate) {
        try {
     ...
     */
    async getPendingFeedbackRequests(patientId) {
        try {
            const [appointments] = await db.execute(`
                SELECT 
                    a.id,
    ...
    getFeedbackCategories() {
        return Object.entries(FeedbackService.FEEDBACK_CATEGORIES).map(([key, value]) => ({
            id: key,
            ...value
        }));
    }
