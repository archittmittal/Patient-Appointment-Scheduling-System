...
class LateArrivalService {
    constructor() {
        ...
        this.checkLateStatus = this.checkLateStatus.bind(this);
        this.getLateArrivalOptions = this.getLateArrivalOptions.bind(this);
        this.getDoctorLatePolicy = this.getDoctorLatePolicy.bind(this);
        this.setDoctorLatePolicy = this.setDoctorLatePolicy.bind(this);
        this.processLateArrival = this.processLateArrival.bind(this);
        this.fitInLatePatient = this.fitInLatePatient.bind(this);
        this.moveToEndOfSession = this.moveToEndOfSession.bind(this);
        this.markForReschedule = this.markForReschedule.bind(this);
        this.logLateArrival = this.logLateArrival.bind(this);
        this.sendPreArrivalReminder = this.sendPreArrivalReminder.bind(this);
        this.getLateArrivalAnalytics = this.getLateArrivalAnalytics.bind(this);
    }

    /**
     * Check if patient is arriving late
     */
    async checkLateStatus(appointmentId, patientId) {
        ...
        const policy = await this.getDoctorLatePolicy(apt.doctor_id);
        ...
        status.options = this.getLateArrivalOptions(status, policy);
     ...
     */
    getLateArrivalOptions(status, policy) {
        const options = [];
     ...
     */
    async getDoctorLatePolicy(doctorId) {
        try {
            ...
            if (policies.length > 0) {
                return { ...this.DEFAULT_POLICIES, ...policies[0] };
            }
        } catch (err) {
     ...
     */
    async setDoctorLatePolicy(doctorId, policyUpdates) {
        const policy = { ...this.DEFAULT_POLICIES, ...policyUpdates };
     ...
     */
    async processLateArrival(appointmentId, patientId, optionId, notes) {
        const status = await this.checkLateStatus(appointmentId, patientId);
        ...
        switch (optionId) {
            case 'proceed':
            case 'fit_in':
                result = await this.fitInLatePatient(appointmentId, patientId, status.minutesLate);
                break;
            case 'end_of_session':
                result = await this.moveToEndOfSession(appointmentId, patientId, status.minutesLate);
                break;
            case 'reschedule':
                result = await this.markForReschedule(appointmentId, patientId, notes);
                break;
            default:
        ...
        await this.logLateArrival(appointmentId, patientId, status.minutesLate, optionId);
     ...
     */
    async fitInLatePatient(appointmentId, patientId, minutesLate) {
     ...
     */
    async moveToEndOfSession(appointmentId, patientId, minutesLate) {
     ...
     */
    async logLateArrival(appointmentId, patientId, minutesLate, handling) {
        try {
            await db.execute(`
     ...
     */
    async sendPreArrivalReminder(appointmentId) {
        const [appointments] = await db.execute(`
            SELECT a.*, 
     ...
     */
    async getLateArrivalAnalytics(doctorId, startDate, endDate) {
        try {
...
module.exports = new LateArrivalService();
