...
class PrepChecklistService {
    constructor() {
        // Ensure methods are bound to this instance
        this.getAppointmentPrep = this.getAppointmentPrep.bind(this);
        this.markPrepComplete = this.markPrepComplete.bind(this);
        this.markPrepIncomplete = this.markPrepIncomplete.bind(this);
        this.addCustomPrepItem = this.addCustomPrepItem.bind(this);
        this.getPatientPrepOverview = this.getPatientPrepOverview.bind(this);
        this.getSpecialtyPrepTemplate = this.getSpecialtyPrepTemplate.bind(this);
        this.sendPrepReminder = this.sendPrepReminder.bind(this);
        this.getDefaultPrepItems = this.getDefaultPrepItems.bind(this);
    }
     ...
     */
    async getAppointmentPrep(appointmentId, patientId) {
        ...
        const defaults = this.getDefaultPrepItems();
     ...
     */
    async markPrepComplete(appointmentId, patientId, itemId) {
        try {
            await db.execute(`
     ...
     */
    async markPrepIncomplete(appointmentId, patientId, itemId) {
        try {
            await db.execute(`
     ...
     */
    async addCustomPrepItem(doctorId, appointmentId, item) {
     ...
     */
    async getPatientPrepOverview(patientId) {
        const [appointments] = await db.execute(`
        ...
        const results = await Promise.all(appointments.map(async (apt) => {
            try {
                const prep = await this.getAppointmentPrep(apt.id, patientId);
                return {
                    ...apt,
                    prepProgress: {
     ...
     */
    getSpecialtyPrepTemplate(specialty) {
        const defaults = this.getDefaultPrepItems();
        return {
            specialty,
     ...
     */
    async sendPrepReminder(appointmentId, patientId) {
        const prep = await this.getAppointmentPrep(appointmentId, patientId);
...
module.exports = new PrepChecklistService();
